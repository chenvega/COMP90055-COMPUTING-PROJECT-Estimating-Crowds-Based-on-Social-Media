...
# Marathon with Twitter Search API
import tweepy
import time
import couchdb
import os
...
def chooseTime(thisTime):
	sinceTime = 'Thu Oct 15 00:00:00 +0000 2015'
			...
			if tweet._json["coordinates"] != None:
				...
				if tweet.id_str not in db:
					if chooseTime(tweet._json['created_at']):
						if getDistance(stadiumLon,stadiumLat,longitude,latitude) < 6:
...
bigT = tweepy.Cursor(api.search, result_type='recent',include_entities=True, geocode="-37.837929,144.962840,6km").items()
while True:
	try:
		tweet = bigT.next()
		if tweet._json["coordinates"] != None:
