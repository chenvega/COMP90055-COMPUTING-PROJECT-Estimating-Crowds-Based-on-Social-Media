...
def chooseTime(thisTime):
	sinceTime = 'Sat Aug 01 00:00:00 +0000 2015'
...
for i in readQuery(path):
	...
	while True:
		try:
			tweet = bigT.next()
			if tweet._json["coordinates"] != None:
				if tweet.id_str not in germanDB:
					germanDB[tweet.id_str] = tweet._json
					screenName = tweet._json["user"]["screen_name"]
					getTimeline(screenName,germanDB,stadiumCoordinates)
		except tweepy.TweepError:
			time.sleep(60)
			continue
		except StopIteration:
